...
//             // The API sends back { token, user }
//             const { token, user } = response.data;

//             // ✅ THIS IS THE FIX: Check for the token and save it
//             if (token) {
//                 // Save the token to the browser's local storage
//                 localStorage.setItem('token', token); 
                
//                 // Now, update the auth context with the user's data
//                 login(user); 
//             } else {
//                 // Handle the unlikely case that login succeeds but no token is sent
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (token) {
                ...
                // We check the user's role to send them to the correct dashboard
